...
public class TouchGrabAgent : Agent
{
    ...
    public override void OnActionReceived(ActionBuffers actionBuffers)
    {
        ...
        else
        {
            ...
            //{
            //    //reward
            //    float distanceToGoal = Vector3.Distance(endEffector.transform.position, goal.transform.position); // roughly 0.7f

            //    var handRotation = endEffector.transform.rotation.eulerAngles;
            //    var jointHeight = 0f; // This is to reward the agent for keeping high up // max is roughly 3.0f
            //    for (int jointIndex = 0; jointIndex < robotController.joints.Length; jointIndex++)
            //    {
            //        jointHeight += robotController.joints[jointIndex].robotPart.transform.position.y - goal.transform.position.y;
            //    }
